...
| After a NAS redeploy, browsers show the old design or fail to login | Stale browser cache. The landlord HTML uses `cache-control: s-maxage=31536000, stale-while-revalidate` and `_next/static/*.css/js` is `immutable, max-age=31536000` (1 year). Browsers serve cached HTML referencing old chunk hashes; the old JS still calls old endpoints. | Hard reload (`Ctrl+Shift+R` / `Cmd+Shift+R`), or open the URL in an Incognito/Private window. Tell users to do this once after every deploy, or accept that they'll see the new design within `stale-while-revalidate` after one background refresh. |
