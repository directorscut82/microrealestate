...
### DEFERRED (need a user decision, a new server payload, or a larger engine change — NOT to be faked)

- **§5 Αχρέωτα tracking + voluntary payment** — needs a new `uncollectedPayments[]`
  subdocument + a POST route + a payment-dialog bucket + receipt label. A real
  feature, not a display tweak.
- **§1.8 repair-vacant Αχρέωτα visibility** — the Αχρέωτα section reads the engine's
  `recipient:'owner' && !ownerBilled` rows, which cover building EXPENSES only;
  the engine does not compute a flag-off repair's vacant share live. Surfacing it
  needs `computeBuildingExpenseBreakdown` (or `getExpenseBreakdown`) to emit a
  repair Αχρέωτα row — a money-surface computation change, not a render tweak. The
  money is correctly NOT billed today; only its *uncollected* visibility is missing.
- **A2 / A6 / left-panel monthly-statement allocation-method inline label** — need a
  new server payload field (tenant rent collected/owed bar; κυμαινόμενα YTD cell)
  or a UI judgment call the user should confirm.
- **OS4/OS6 owner-tab CSV** — owner-side export.
