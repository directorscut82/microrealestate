...
| **P2** | typography | Surface (81,44), ATAK (00998877780), ΔΕΗ supply number render in proportional Inter, not the mandated tabular mono numeric role. | PropertyForm fields | Apply `font-mono tabular-nums` to Surface, ATAK, plot-surface, ΔΕΗ inputs. |
...
| **P2** | cramping (was P1) | ΧΡΕΩΣΕΙΣ breakdown is a wall of ~13 near-identical rows (same tenant ΔΟΚΙΜΗ ΒΗΤΑ, same expense, mostly 4,86 €). Monotonous identical-stack anti-pattern. | `BuildingExpensePanel.js` L815-853 | Group by recipient: tenant name once as a section header, units beneath, or collapse to "ΔΟΚΙΜΗ ΒΗΤΑ — 13 units, … σύνολο 63,18 €" with expand. |
