...
interface EmailMessage {
  ...
  text: string;
  html: string;
  attachment: Array<{ filename: string; data: Buffer }>;
}
...
async function _sendWithMailgun(config: any, email: EmailMessage): Promise<EmailResult> {
...
async function _sendWithSmtp(config: any, email: EmailMessage): Promise<EmailResult> {
  const { replyTo, from, to, bcc, subject, text, html, attachment } = email;
...
export function sendEmail(email: EmailMessage, data: any): Promise<EmailResult> {
