...
// Parse "Όροφος 3 ΟΔΟΣ ΖΗΤΑ 9 19947 ΔΟΚΙΜΑΙΟΥ, ΑΘΗΝΩΝ (ΝΟΜΑΡΧΙΑ)"
...
function parseAddress(raw: string): ParsedAddress {
  ...
  // tokens were silently leaking into street1 (e.g. "ΟΔΟΣ ΖΗΤΑ 9 Σοφίτα").
  const FLOOR_RE =
  ...
  if (anchored) {
    ...
    // City and state are typically genitive (ΔΟΚΙΜΑΙΟΥ → ΔΟΚΙΜΑΙ,
