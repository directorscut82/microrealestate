...
function parseE9Row(
  ...
  const afterAtak = rowText.substring(12).trim(); // "001122 33430 " = 12 chars
  ...
  // Municipality is in genitive (ΔΟΚΙΜΑΙΟΥ, ΑΘΗΝΑΙΩΝ, ΝΕΑΣ ΔΟΚΙΜΑΝΤΟΣ)
  ...
  if (municipalityAndStreet) {
    ...
    // "ΔΟΚΙΜΩΝΩΝ ΑΛΦΑΙΟΥ - ΠΑΡΑΛΙΑ ΠΕΡΙΟΧΗ ΘΗΤΑ ΑΓ. ΣΙΓΜΑΤΙΑΣ" → street
    // "ΑΓ. ΣΙΓΜΑΤΙΑΣ", municipality "ΔΟΚΙΜΩΝΩΝ ΑΛΦΑΙΟΥ - ΠΑΡΑΛΙΑ
    // ΠΕΡΙΟΧΗ ΘΗΤΑ".
  ...
  if (hasX) {
    ...
    if (xPos >= 0) {
      ...
      if (streetsMatch) {
        ...
        // prefix to the next token so e.g. "ΑΓ. ΣΙΓΜΑΤΙΑΣ" stays
...
export function parseE9(text: string): ParsedE9Result {
  ...
  // groups (e.g. ΟΔΟΣ ΖΗΤΑ 9 with one zip-less row alongside zipped
