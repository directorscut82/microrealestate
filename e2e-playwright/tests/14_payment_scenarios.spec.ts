 ...
 *
 * Drives DOKIMASTI's real rent through the full UI for several payment
 * shapes, each time:
...
const TENANT_NAME = 'DOKIMASTI E2ETEST';
...
async function openDokimastiDialog(page: import('@playwright/test').Page) {
...
test('scenario 1 · single rent-only payment, auto-spread mode', async ({
  ...
  await signIn(page);
  await openDokimastiDialog(page);
...
test('scenario 4 · KPI tile shows (prior dues: X) when carry-in exists', async ({
  ...
  // DOKIMASTI + MISTHOTIS have carry-in arrears in May 2026 (sum 2182 €
...
test('scenario 6 · express dialog offers prior-balance row for tenants in arrears', async ({
  ...
  // Drawer renders right side. Look for a row mentioning DOKIMASTI with
  ...
  // least once (DOKIMASTI has carry-in).
  expect(
    drawerHtml,
