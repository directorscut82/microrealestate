 ...
 *
 * Asserts:
 *   1. Opening the dialog on DOKIMASTI's already-paid rent shows a
 ...
 *   5. DOKIMASTI's data is restored at the end so we don't drift the
 *      user's real data.
 */
import { test, expect, request } from '@playwright/test';

const REAL_EMAIL = 'e2elandlord82@gmail.com';
const REAL_PASSWORD = 'Passcode@1234';
...
test('Approach A: DOKIMASTI dialog shows locked tile, add-another flow works', async ({ page }) => {
  ...
  // Find DOKIMASTI's row.
  const nameSpan = page.locator('span.text-lg.font-medium', { hasText: 'DOKIMASTI E2ETEST' }).first();
  ...
  // ---- restore DOKIMASTI's original state ----
  // Reset to the original 1 payment of 120 cash on 22/05.
  const restoreScript = `
    var t = db.occupants.findOne({name: "DOKIMASTI E2ETEST"});
  ...
  console.log('NOTE: Restore DOKIMASTI state externally:', restoreScript);
});
