...
test.describe.serial('V_T1: E9 grouping + outcome reporting', () => {
  ...
  test('V_T1.1: dragging all 5 E9 PDFs collapses duplicates — ΟΔΟΣ ΖΗΤΑ 9 shows once', async ({
    page
  }) => {
    ...
    // ΟΔΟΣ ΖΗΤΑ 9 must appear EXACTLY ONCE in the dialog (was 4× pre-fix).
    // It is rendered as part of "ΟΔΟΣ ΖΗΤΑ 9, ΔΟΚΙΜΑΙ" in the building
    // card heading.
    const odos zitaCount = (previewText.match(/ΟΔΟΣ ΖΗΤΑ\s+9/g) || []).length;
    expect(odos zitaCount, 'ΟΔΟΣ ΖΗΤΑ 9 appears exactly once').toBe(1);

    // ΟΔΟΣ ΖΗΤΑ 9 should have the merged unit count: 2 (from 2027-1) + 2 (from
    ...
    // Find the ΟΔΟΣ ΖΗΤΑ block and extract its "{N} units" / "{N} μονάδες".
    const odos zitaBlock = previewText.match(
      /ΟΔΟΣ ΖΗΤΑ\s+9[\s\S]*?((?:\d+)\s*(?:units?|μονάδα|μονάδες))/
    );
    expect(odos zitaBlock, 'ΟΔΟΣ ΖΗΤΑ 9 unit count line present').toBeTruthy();
    const unitCount = Number(odos zitaBlock![1].match(/\d+/)![0]);
    expect(unitCount, 'ΟΔΟΣ ΖΗΤΑ 9 merges multiple PDFs (>=3 units)').toBeGreaterThanOrEqual(3);
    ...
    //   ΟΔΟΣ ΖΗΤΑ 167 (19946)
    //   ΟΔΟΣ ΗΤΑ 24 (19947)        — appears in 2026-3 + 2027-5
    //   ΟΔΟΣ ΖΗΤΑ 9 (19947)      — appears in 2027-1, -2, -4, -5
    //   ΑΓ. ΟΔΟΣ ΕΨΙΛΟΝ 28 (19943)
    //   ΚΑΤΩ ΚΛΕΙΣΜΑ 0 (no zip)
    //   ΠΕΡΙΟΧΗ ΘΗΤΑ block-plot (post-hotfix T2.P1.3 — surfaces as a building
    //                        rather than landing silently in skippedLandPlots)
    // = 6 unique. Pre-hotfix this was 5 because the ΠΕΡΙΟΧΗ ΘΗΤΑ /
  ...
  test('V_T1.2 + V_T1.4 + V_T1.5: 2027-5 alone — ΟΔΟΣ ΖΗΤΑ 9 = 1 building, city cleaned, auxSurface=0, yearBuilt=1896', async ({
    page
  }) => {
    test.setTimeout(180_000);
    ...
    // KATΩ ΚΛΕΙΣΜΑ row 00112233414 inside the preview buildings.
    const allUnits: any[] = (parseBody.buildings || []).flatMap(
      (b: any) => b.units || []
    );
    const targetUnit = allUnits.find(
      (u: any) => u.atakNumber === '00112233414'
    );
    expect(
      targetUnit,
      `parser returned unit 00112233414; got ${allUnits.map((u: any) => u.atakNumber).join(', ')}`
    ).toBeTruthy();
    console.log(
    ...
    // ===== V_T1.2: ΟΔΟΣ ΖΗΤΑ 9 must be ONE building in mongo =====
    const odos zitaCount = mongoExec(
      `print(db.buildings.find({realmId: "${realmId}", "address.street1": "ΟΔΟΣ ΖΗΤΑ 9"}).count());`
    );
    expect(odos zitaCount, 'mongo readback available').not.toBeNull();
    console.log(`==== T1.2 ΟΔΟΣ ΖΗΤΑ 9 mongo count = "${odos zitaCount}" ====`);
    expect(Number(odos zitaCount)).toBe(1);

    // ===== V_T1.2: every building's city is the cleaned 'ΔΟΚΙΜΑΙ' =====
    // 2027-5 has 3 buildings: ΟΔΟΣ ΖΗΤΑ 9 / ΚΑΤΩ ΚΛΕΙΣΜΑ 0 / ΟΔΟΣ ΗΤΑ 24.
    // ΟΔΟΣ ΖΗΤΑ + ΟΔΟΣ ΗΤΑ are in ΔΟΚΙΜΑΙ; ΚΑΤΩ ΚΛΕΙΣΜΑ is rural and uses
    // the settlement name as city. The fix cleans the genitive
    // (ΔΟΚΙΜΑΙΟΥ → ΔΟΚΙΜΑΙ) on every row regardless of zip resolution.
    ...
    const cityRowsRaw = mongoExec(`
      var s = db.buildings.findOne({realmId: "${realmId}", "address.street1": "ΟΔΟΣ ΖΗΤΑ 9"});
      var k = db.buildings.findOne({realmId: "${realmId}", "address.street1": "ΟΔΟΣ ΗΤΑ 24"});
    ...
    for (const row of [cityRows.s, cityRows.k]) {
      ...
      expect(
        /ΔΟΚΙΜΑΙ$/.test(row.c),
        `city '${row.c}' for ${row.a} ends with cleaned 'ΔΟΚΙΜΑΙ' (not 'ΔΟΚΙΜΑΙΟΥ')`
      ).toBe(true);
      // Anti-regression: must NOT still be the genitive.
      expect(row.c, `${row.a} city is not genitive`).not.toMatch(/ΔΟΚΙΜΑΙΟΥ/);
    }
