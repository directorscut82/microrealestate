 ...
 *  - V_T2.6 (P1.3)  — settlement block-plot ATAK 001122 33393 surfaces as building
...
test.describe.serial('V_T2: E9 import behavioural fixes', () => {
  ...
  test('V_T2.5 (P1.4+P1.14): import 2027-5 → unit owners[] has 2 entries, rightType=full', async () => {
    ...
    // Round-trip to mongo: find unit with atakNumber 00112233414 and
    // assert owners[].length === 2 and rightType === 'full'.
    const mongoOut = mongoExec(`
      var b = db.buildings.findOne({"realmId": "${realmId}", "units.atakNumber": "00112233414"});
      if (!b) { print(JSON.stringify({error:"no building"})); quit(); }
      var u = b.units.find(function(x){ return x.atakNumber === "00112233414"; });
  ...
  test('V_T2.6 (P1.3): 2027-1.pdf surfaces ΠΕΡΙΟΧΗ ΘΗΤΑ ATAK 00112233393 as a building', async () => {
    test.setTimeout(120_000);
    ...
    // The parser should emit ATAK 00112233393 as a unit on a building
    // (NOT silently dropped via skippedLandPlots). ΠΕΡΙΟΧΗ ΘΗΤΑ row.
    ...
    const perioxith = allUnits.find((u) => u.atakNumber === '00112233393');
    expect(perioxith, 'ATAK 00112233393 surfaces as unit').toBeTruthy();
    expect(
      String(perioxith!.city || '').includes('ΠΕΡΙΟΧΗ ΘΗΤΑ'),
      `building city contains ΠΕΡΙΟΧΗ ΘΗΤΑ — got ${perioxith!.city}`
    ).toBe(true);
  });
