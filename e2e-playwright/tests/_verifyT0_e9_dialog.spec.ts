...
test.describe.serial('V_T0: ImportE9Dialog hardening', () => {
  ...
  test('V_T0.1: owner row renders "<Last> <First> (ΑΦΜ: <id>)" with no leading blank', async ({
    ...
    // Upload PDF for ΔΟΚΙΜΗ ΓΕΩΡΓΙΟΣ (taxId 125479189)
    ...
    const expected = 'ΔΟΚΙΜΗ ΓΕΩΡΓΙΟΣ (ΑΦΜ: 125479189)';
  ...
  test('V_T0.2: 1-unit building renders "1 unit" + "1 land plot skipped" — not "1 units"', async ({
    ...
    // PeriousiakiKatastasi2026-3.pdf yields ΟΔΟΣ ΖΗΤΑ 167 (1 unit) +
    // ΟΔΟΣ ΗΤΑ 24 (5 units), skippedLandPlots=4.
    // Use 2027-2.pdf instead — same owner has 1 building with 1 unit
    // (ΟΔΟΣ ΖΗΤΑ 9) and skippedLandPlots=1 to exercise the
    ...
    expect(previewText, 'plural "5 units" for ΟΔΟΣ ΗΤΑ').toContain('5 units');
    ...
    expect(previewText, 'plural "5 μονάδες" for ΟΔΟΣ ΗΤΑ (el)').toContain('5 μονάδες');
