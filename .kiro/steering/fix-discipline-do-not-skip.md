...
- **Declaring a fix "done" that adversarial review then breaks.** This is the live failure mode. In the June 2026 vacant-owner money batch, the fix was declared done at `a6fd2567`; refute-by-default review workflows then found FIXED-ZERO method-flip, single_unit stale-propertyId, sub-cent rounding, and duplicate-propertyId — four real money bugs, each surfacing AFTER "done" was claimed. Re-reading this document would not have caught any of them. The adversarial workflow did.
