...
# 8. All 11 expected services present (voiceasr added 2026-08 — shadow-mode voice recognition)
for svc in redis mongo gateway authenticator api tenantapi emailer pdfgenerator voiceasr landlord-frontend tenant-frontend; do
...
# 11. Memory limits present on every service (11 services)
mem_limit_count=$(grep -c 'mem_limit:' "$FILE" || true)
check "All 11 services have mem_limit (found $mem_limit_count)" \
  bash -c "[[ '$mem_limit_count' == '11' ]]"

# 12. restart policy on every service
restart_count=$(grep -c 'restart: unless-stopped' "$FILE" || true)
check "All 11 services have restart: unless-stopped (found $restart_count)" \
  bash -c "[[ '$restart_count' == '11' ]]"
