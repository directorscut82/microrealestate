...
# 8. All 10 expected services present
for svc in redis mongo gateway authenticator api tenantapi emailer pdfgenerator landlord-frontend tenant-frontend; do
...
# 11. Memory limits present on every service (10 services)
mem_limit_count=$(grep -c 'mem_limit:' "$FILE" || true)
check "All 10 services have mem_limit (found $mem_limit_count)" \
  bash -c "[[ '$mem_limit_count' == '10' ]]"

# 12. restart policy on every service
restart_count=$(grep -c 'restart: unless-stopped' "$FILE" || true)
check "All 10 services have restart: unless-stopped (found $restart_count)" \
  bash -c "[[ '$restart_count' == '10' ]]"
